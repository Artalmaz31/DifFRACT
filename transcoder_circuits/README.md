...
| File | What it holds |
|---|---|
| `backends.py` | Per-architecture adapters (`TRACING_ARCHS`): block wiring, joint-attention recomputation, prompt aliases, latent decoding. |
...
| `pipeline.py` | `LRMPipeline`, the end-to-end driver that traces, aggregates, prunes and validates a circuit and writes it to JSON. |
...
| `interventions.py` | Circuit-guided steering (per-feature, per-step, per-stream, per-CFG-branch). |
